...
export function CopyCommand({ command }: { command: string }) {
  ...
  async function copy() {
    try {
      ...
      console.warn("[copy-command] clipboard write failed:", String(err));
    }
  }

  return (
    <div className="rounded-2xl bg-raise-1 p-1.5 ring-1 ring-line-subtle">
      <div className="flex items-center gap-3 rounded-[calc(1.25rem-0.375rem)] bg-base/60 px-4 py-3.5 shadow-[inset_0_1px_0_oklch(100%_0_0/0.06)]">
        <span aria-hidden className="select-none font-mono text-sm text-accent">
          $
        </span>
        <code className="flex-1 overflow-x-auto whitespace-nowrap font-mono text-sm text-ink-body [font-variant-ligatures:none]">
          {command}
        </code>
        <button
          type="button"
