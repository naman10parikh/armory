#!/usr/bin/env node
// ============================================================================
// crawl-ecc.mjs — Deterministic mega-crawl of affaan-m/ecc into engram stubs.
// ----------------------------------------------------------------------------
// Ingests the COMPLETE affaan-m/ecc repo (the largest multi-component harness
// library) into incoming/ecc-full/ — every component, not a subset:
...
// engram markdown files via toMarkdown() from crawl.mjs. Idempotent: re-running
// overwrites the same files (name === filename, unique within type).
//
// Run:  node ingest/crawl-ecc.mjs            (writes incoming/ecc-full/)
//       node ingest/crawl-ecc.mjs --dry-run  (prints counts only)
// ============================================================================
import {
...
const OUT = join(ROOT, "incoming", "ecc-full");
...
// ---------------------------------------------------------------------------
// Write all stubs grouped by type into incoming/ecc-full/<type>/<name>.md.
// ---------------------------------------------------------------------------
...
function main() {
  ...
  console.log(`${dryRun ? "[dry-run] " : ""}ecc-full → ${OUT}`);
