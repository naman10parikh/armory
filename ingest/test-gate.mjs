...
// top ranked row, or no score at all. Pure: pass stack.json and the rows computeRows made. One line per gap.
export function stackPickGaps(stack, rows) {
  ...
  for (const c of stack.components ?? []) {
    ...
    for (const p of c.picks ?? []) {
      ...
      if (below && !String(p.reason ?? "").trim()) gaps.push(`${c.slug}: ${p.armoryName} sits below ${top?.name ?? "the top row"} and has no reason`);
    }
  }
  return gaps;
}
...
function main(argv) {
  ...
  if (stack) {
    console.log(`test-gate: ${stack.picks} /stack picks · ${stack.gaps.length} off their shelf or below its top row without a reason`);
  ...
  if (stack?.gaps.length) { console.error(`::error:: ${stack.gaps.length} /stack pick gaps: give each a reason in web/src/data/stack.json, or put it back on its shelf.`); process.exit(1); }
