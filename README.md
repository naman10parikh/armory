...
```
brain/              📚 The catalog  — metadata + detail pages for all 25,138 components
  components/mcps/  └─ MCP details → brain/components/mcps/<slug>.md  (~21,733 files)
catalog.json        └─ generated index (never hand-edited)

skills/ agents/     🧰 The gear     — actual installable files (~2,546) at repo root
commands/ hooks/ rules/
mcp/                └─ install configs for all ~21,000 MCPs (generated by ingest/surface.mjs)
