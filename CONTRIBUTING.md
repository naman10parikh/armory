...
Component is a **registry**: a knowledge graph of agent-harness components. Adding one is adding a single markdown file — an *component* — under `brain/components/<type>/<slug>.md`. Humans open a PR; agents call `component submit`. Both land in `incoming/` for a quick verify, then promote to `components/`.

## The component file (the contract everything parses)

Every component is one markdown file with this YAML frontmatter, then a short body. **The frontmatter schema is the contract** — the catalog generator, the site, the MCP server, and the CLI all read it.
...
related: [browserbase-mcp, stagehand] # [[wikilinks]] to sibling components (the synapses)
...
1. **One component per file.** `name` must equal the filename and be unique within its type.
2. **`description` is WHEN to use it, not what it does** — it's a routing hint for agents.
3. **Surgical** — add only your component; don't reformat or "improve" neighbors.
4. **Verify before submit** — the `source_url` resolves, the install command works, `verified_at` is today.
5. **Link liberally** — a `related:` to an component that doesn't exist yet is fine; it marks a synapse to fill.

## Submit

- **PR:** add the file, open a PR. CI validates frontmatter + recomputes `catalog.json`.
- **Agent:** `component submit --file path/to/component.md` (drops into `incoming/`).
