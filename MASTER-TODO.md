...
- [x] **C3** (f1737e8c) **Time-slider** DONE + browser-verified — the chairman's signature "drag a slider and watch it grow over time." `order` (0..1 by `verified_at`) per node in `lib/graph.ts`; `reveal` prop on `SynapseGraph` draws only nodes/edges ≤ cutoff (pre-settled layout → revealed nodes land in final position = growth, no re-jiggle); "GREW TO" range slider + live `<date> · N/M` label in `graph-client.tsx`. Zero-dep, tsc clean. Proven: dragging left went `2026-05-31·324/324` → `2026-05-26·97/324`, graph visibly winds back. (Future polish: a play/animate button + buckets keyed to AUTOLAB-LOG nightly entries.)
