---
name: whoiser-whois
type: mcps
description: >
  Provides a lightweight server for performing WHOIS lookups using the whoiser library, enabling retrieval of domain registration and ownership information through TypeScript-based type-safe queries.
source_repo: modelcontextprotocol-servers/whois-mcp
source_url: https://github.com/modelcontextprotocol-servers/whois-mcp
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 21
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 4
pushed_at: "2025-03-07T10:13:19Z"
---
## What it is
MCP server `Whoiser`, catalogued on PulseMCP. Provides a lightweight server for performing WHOIS lookups using the whoiser library, enabling retrieval of domain registration and ownership information through TypeScript-based type-safe queries.

## When to use it
Provides a lightweight server for performing WHOIS lookups using the whoiser library, enabling retrieval of domain registration and ownership information through TypeScript-based type-safe queries.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/modelcontextprotocol-servers/whois-mcp

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/whoiser-whois). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
