---
name: truss44-crypto-price-coincap
type: mcps
description: >
  Integrates with CoinCap's API to provide real-time cryptocurrency data, enabling price tracking, market analysis, and historical trend examination for financial applications.
source_repo: truss44/mcp-crypto-price
source_url: https://github.com/truss44/mcp-crypto-price
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 40
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 8
pushed_at: "2026-08-31T17:32:12Z"
---
## What it is
MCP server `Crypto Price (CoinCap)`, catalogued on PulseMCP. Integrates with CoinCap's API to provide real-time cryptocurrency data, enabling price tracking, market analysis, and historical trend examination for financial applications.

## When to use it
Integrates with CoinCap's API to provide real-time cryptocurrency data, enabling price tracking, market analysis, and historical trend examination for financial applications.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/truss44/mcp-crypto-price

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/truss44-crypto-price-coincap). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
