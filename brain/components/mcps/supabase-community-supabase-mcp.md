---
name: supabase-community-supabase-mcp
type: mcps
description: >
  Official Supabase MCP server to connect AI assistants directly with your Supabase project and allows them to perform tasks like managing tables, fetching config, and querying data.
source_repo: supabase-community/supabase-mcp
source_url: https://github.com/supabase-community/supabase-mcp
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
verified_at: 2026-05-26
related: []
tags: [mcp, databases]
stars: 2886
forks: 402
pushed_at: "2026-09-01T17:26:43Z"
---
## What it is
Official Supabase MCP server to connect AI assistants directly with your Supabase project and allows them to perform tasks like managing tables, fetching config, and querying data.

## When to use it
When an agent needs the "Databases" capability this MCP server exposes.

## Source
Migrated from the awesome-mcp-servers navigation directory (category: Databases). See https://github.com/supabase-community/supabase-mcp. Pending verify -> promote.
