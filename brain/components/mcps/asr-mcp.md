---
name: asr-mcp
type: mcps
description: >
  MCP server for local speech-to-text using Whisper Large V3 (MLX), enabling audio transcription with text/timestamps/SRT output and LLM-based correction, all running offline on Apple Silicon.
source_repo: fengredrum/asr-mcp
source_url: https://github.com/fengredrum/asr-mcp
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-08-02T23:16:28Z"
---
## What it is
MCP server for local speech-to-text using Whisper Large V3 (MLX), enabling audio transcription with text/timestamps/SRT output and LLM-based correction, all running offline on Apple Silicon.

## When to use it
MCP server for local speech-to-text using Whisper Large V3 (MLX), enabling audio transcription with text/timestamps/SRT output and LLM-based correction, all running offline on Apple Silicon.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/dquwe51pit) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
