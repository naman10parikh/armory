---
name: ntnu-mcp
type: mcps
description: >
  A read-only MCP server exposing NTNU course data to LLM assistants, enabling course catalog search, teaching schedules, weekly timetables, grade statistics, and course comparison tools.
source_repo: MartinSA04/ntnu-mcp
source_url: https://github.com/MartinSA04/ntnu-mcp
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 1
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-07-28T11:33:04Z"
---
## What it is
A read-only MCP server exposing NTNU course data to LLM assistants, enabling course catalog search, teaching schedules, weekly timetables, grade statistics, and course comparison tools.

## When to use it
A read-only MCP server exposing NTNU course data to LLM assistants, enabling course catalog search, teaching schedules, weekly timetables, grade statistics, and course comparison tools.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/svlm59cn19) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
