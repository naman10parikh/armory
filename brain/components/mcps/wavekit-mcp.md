---
name: wavekit-mcp
type: mcps
description: >
  An MCP server that provides AI assistants with a persistent, sandboxed Python environment for waveform analysis, enabling loading and manipulation of VCD/FST/FSDB files and temporal pattern matching.
source_repo: cxzzzz/wavekit-mcp
source_url: https://github.com/cxzzzz/wavekit-mcp
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 9
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-07-27T16:22:17Z"
---
## What it is
An MCP server that provides AI assistants with a persistent, sandboxed Python environment for waveform analysis, enabling loading and manipulation of VCD/FST/FSDB files and temporal pattern matching.

## When to use it
An MCP server that provides AI assistants with a persistent, sandboxed Python environment for waveform analysis, enabling loading and manipulation of VCD/FST/FSDB files and temporal pattern matching.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/u4sio790dn) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
