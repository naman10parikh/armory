---
name: thrashr888-terraform-registry
type: mcps
description: >
  Integrates with the Terraform Registry API to enable provider lookup, resource usage examples, module recommendations, and schema details retrieval for infrastructure-as-code development.
source_repo: thrashr888/terraform-mcp-server
source_url: https://github.com/thrashr888/terraform-mcp-server
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 120
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 20
pushed_at: "2025-05-20T18:54:50Z"
---
## What it is
MCP server `Terraform Registry`, catalogued on PulseMCP. Integrates with the Terraform Registry API to enable provider lookup, resource usage examples, module recommendations, and schema details retrieval for infrastructure-as-code development.

## When to use it
Integrates with the Terraform Registry API to enable provider lookup, resource usage examples, module recommendations, and schema details retrieval for infrastructure-as-code development.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/thrashr888/terraform-mcp-server

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/thrashr888-terraform-registry). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
