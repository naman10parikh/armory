---
name: oriole
type: mcps
description: >
  This MCP server exposes 16 bilingual tools for operating a review-gated China job-source intelligence engine, enabling discovery, probing, human approval, collection, and job/region queries. It lets an LLM manage the Oriole job pipeline over stdio without requiring model access to credentials.
source_repo: gaofeibilly-maker/oriole-job-engine
source_url: https://github.com/gaofeibilly-maker/oriole-job-engine
license: Apache 2.0
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
This MCP server exposes 16 bilingual tools for operating a review-gated China job-source intelligence engine, enabling discovery, probing, human approval, collection, and job/region queries. It lets an LLM manage the Oriole job pipeline over stdio without requiring model access to credentials.

## When to use it
This MCP server exposes 16 bilingual tools for operating a review-gated China job-source intelligence engine, enabling discovery, probing, human approval, collection, and job/region queries. It lets an LLM manage the Oriole job pipeline over stdio without requiring model access to credentials.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/fp96yr0w3a) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
