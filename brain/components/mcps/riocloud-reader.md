---
name: riocloud-reader
type: mcps
description: >
  Enables AI agents to fetch and digest content from 30+ platforms (Twitter, YouTube, Reddit, etc.) via a unified API. Supports multi-format output, transcription, and direct Obsidian sync.
source_repo: Riocloud/riocloud-reader
source_url: https://github.com/Riocloud/riocloud-reader
license: BSD 2-Clause "Simplified" License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 18
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 4
pushed_at: "2026-02-25T04:44:17Z"
---
## What it is
Enables AI agents to fetch and digest content from 30+ platforms (Twitter, YouTube, Reddit, etc.) via a unified API. Supports multi-format output, transcription, and direct Obsidian sync.

## When to use it
Enables AI agents to fetch and digest content from 30+ platforms (Twitter, YouTube, Reddit, etc.) via a unified API. Supports multi-format output, transcription, and direct Obsidian sync.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/k5d0s1uhr6) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
