---
name: tqs-mcp
type: mcps
description: >
  MCP server for TQS structural engineering CAD/analysis software, enabling Claude to read building models, extract drawing data, run structural processing, and script against the TQS API.
source_repo: carlostavares-manager-MNGT/tqs-mcp
source_url: https://github.com/carlostavares-manager-MNGT/tqs-mcp
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
MCP server for TQS structural engineering CAD/analysis software, enabling Claude to read building models, extract drawing data, run structural processing, and script against the TQS API.

## When to use it
MCP server for TQS structural engineering CAD/analysis software, enabling Claude to read building models, extract drawing data, run structural processing, and script against the TQS API.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/navxen0env) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
