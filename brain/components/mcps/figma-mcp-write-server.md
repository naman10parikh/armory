---
name: figma-mcp-write-server
type: mcps
description: >
  An MCP server that provides write access to Figma through the Plugin API, enabling AI agents to create, modify, and manage Figma designs programmatically.
source_repo: oO/figma-mcp-write-server
source_url: https://github.com/oO/figma-mcp-write-server
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 23
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 10
pushed_at: "2025-11-25T16:29:19Z"
---
## What it is
An MCP server that provides write access to Figma through the Plugin API, enabling AI agents to create, modify, and manage Figma designs programmatically.

## When to use it
An MCP server that provides write access to Figma through the Plugin API, enabling AI agents to create, modify, and manage Figma designs programmatically.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/v1y0w4fpon) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
