---
name: box-mcp-server-box-remote
type: mcps
description: >
  The Box MCP server allows third party AI agents to securely and seamlessly access Box content and use tools such as search, asking questions from files and folders, and data extraction.
source_repo: box/mcp-server-box-remote
source_url: https://github.com/box/mcp-server-box-remote/
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
verified_at: 2026-05-26
related: []
tags: [mcp, file-systems]
stars: 6
forks: 8
pushed_at: "2025-09-20T03:58:32Z"
---
## What it is
The Box MCP server allows third party AI agents to securely and seamlessly access Box content and use tools such as search, asking questions from files and folders, and data extraction.

## When to use it
When an agent needs the "File Systems" capability this MCP server exposes.

## Source
Migrated from the awesome-mcp-servers navigation directory (category: File Systems). See https://github.com/box/mcp-server-box-remote/. Pending verify -> promote.
