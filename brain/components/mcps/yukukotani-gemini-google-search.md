---
name: yukukotani-gemini-google-search
type: mcps
description: >
  Provides Google Search functionality through Gemini's native grounding capabilities, delivering search results with automatic source citations and grounding metadata for reliable information retrieval.
source_repo: yukukotani/mcp-gemini-google-search
source_url: https://github.com/yukukotani/mcp-gemini-google-search
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 82
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 5
pushed_at: "2025-07-20T02:00:22Z"
---
## What it is
MCP server `Google Search (Gemini)`, catalogued on PulseMCP. Provides Google Search functionality through Gemini's native grounding capabilities, delivering search results with automatic source citations and grounding metadata for reliable information retrieval.

## When to use it
Provides Google Search functionality through Gemini's native grounding capabilities, delivering search results with automatic source citations and grounding metadata for reliable information retrieval.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/yukukotani/mcp-gemini-google-search

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/yukukotani-gemini-google-search). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
