---
name: fundgraph
type: mcps
description: >
  An MCP server that gives investment teams an entity-resolved knowledge graph of people and organizations, enabling graph-based retrieval like warm introductions and relationship analysis from scattered data sources.
source_repo: clippycommits/fundgraph
source_url: https://github.com/clippycommits/fundgraph
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 1
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-08-10T20:12:38Z"
---
## What it is
An MCP server that gives investment teams an entity-resolved knowledge graph of people and organizations, enabling graph-based retrieval like warm introductions and relationship analysis from scattered data sources.

## When to use it
An MCP server that gives investment teams an entity-resolved knowledge graph of people and organizations, enabling graph-based retrieval like warm introductions and relationship analysis from scattered data sources.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/mosobi9u1h) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
