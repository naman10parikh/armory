---
name: acmt001-mcp
type: mcps
description: >
  MCP server for ISO 20022 acmt.001 Account Opening (and companion acmt.* messages): message-type discovery, required-field lookup, JSON Schema introspection, IBAN/BIC/LEI validation, flat-record validation, and validated acmt XML generation.
source_repo: sebastienrousseau/acmt001-mcp
source_url: https://github.com/sebastienrousseau/acmt001-mcp
license: Apache 2.0
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 1
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-08-29T17:47:25Z"
---
## What it is
MCP server for ISO 20022 acmt.001 Account Opening (and companion acmt.* messages): message-type discovery, required-field lookup, JSON Schema introspection, IBAN/BIC/LEI validation, flat-record validation, and validated acmt XML generation.

## When to use it
MCP server for ISO 20022 acmt.001 Account Opening (and companion acmt.* messages): message-type discovery, required-field lookup, JSON Schema introspection, IBAN/BIC/LEI validation, flat-record validation, and validated acmt XML generation.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/jx7m14lorf) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
