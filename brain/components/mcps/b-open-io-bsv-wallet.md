---
name: b-open-io-bsv-wallet
type: mcps
description: >
  Integrates with Bitcoin SV blockchain to enable wallet operations, transaction management, and NFT interactions while maintaining local private key security
source_repo: b-open-io/bsv-mcp
source_url: https://github.com/b-open-io/bsv-mcp
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 21
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 8
pushed_at: "2026-07-30T19:29:27Z"
---
## What it is
MCP server `Bitcoin SV Wallet`, catalogued on PulseMCP. Integrates with Bitcoin SV blockchain to enable wallet operations, transaction management, and NFT interactions while maintaining local private key security

## When to use it
Integrates with Bitcoin SV blockchain to enable wallet operations, transaction management, and NFT interactions while maintaining local private key security

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/b-open-io/bsv-mcp

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/b-open-io-bsv-wallet). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
