---
name: apw124-logseq
type: mcps
description: >
  Enables AI interaction with Logseq knowledge graphs for capturing notes, organizing information, and retrieving knowledge from personal databases
source_repo: apw124/logseq-mcp
source_url: https://github.com/apw124/logseq-mcp
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 15
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 6
pushed_at: "2025-07-02T12:00:05Z"
---
## What it is
MCP server `Logseq`, catalogued on PulseMCP. Enables AI interaction with Logseq knowledge graphs for capturing notes, organizing information, and retrieving knowledge from personal databases

## When to use it
Enables AI interaction with Logseq knowledge graphs for capturing notes, organizing information, and retrieving knowledge from personal databases

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/apw124/logseq-mcp

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/apw124-logseq). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
