---
name: atracker-mcp-server
type: mcps
description: >
  Syncs ATracker time tracking data to a local SQLite database and provides MCP tools for querying tasks, entries, tags, and exporting data.
source_repo: braveva/ATracker-MCP
source_url: https://github.com/braveva/ATracker-MCP
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Syncs ATracker time tracking data to a local SQLite database and provides MCP tools for querying tasks, entries, tags, and exporting data.

## When to use it
Syncs ATracker time tracking data to a local SQLite database and provides MCP tools for querying tasks, entries, tags, and exporting data.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/rshx4de5ba) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
