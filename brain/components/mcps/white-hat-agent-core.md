---
name: white-hat-agent-core
type: mcps
description: >
  A model-neutral cyber capability brain for AI agents and human researchers, providing MCP tools for knowledge ingestion, composition, campaign planning, fleet management, evidence binding, and discovery replay.
source_repo: kappa9999/white-hat-agent
source_url: https://github.com/kappa9999/white-hat-agent
license: Apache 2.0
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
A model-neutral cyber capability brain for AI agents and human researchers, providing MCP tools for knowledge ingestion, composition, campaign planning, fleet management, evidence binding, and discovery replay.

## When to use it
A model-neutral cyber capability brain for AI agents and human researchers, providing MCP tools for knowledge ingestion, composition, campaign planning, fleet management, evidence binding, and discovery replay.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/cj9iy3foze) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
