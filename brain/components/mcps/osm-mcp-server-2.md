---
name: osm-mcp-server-2
type: mcps
description: >
  An OpenStreetMap MCP server that provides location-based services including geocoding, route directions, points of interest search, and neighborhood analysis to enhance LLMs with geospatial capabilities.
source_repo: Sowjanyakaki/OpenStreetMap_MCP
source_url: https://github.com/Sowjanyakaki/OpenStreetMap_MCP
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
An OpenStreetMap MCP server that provides location-based services including geocoding, route directions, points of interest search, and neighborhood analysis to enhance LLMs with geospatial capabilities.

## When to use it
An OpenStreetMap MCP server that provides location-based services including geocoding, route directions, points of interest search, and neighborhood analysis to enhance LLMs with geospatial capabilities.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/wtle5rjehm) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
