---
name: openmf-mcp-mifosx
type: mcps
description: >
  A core banking integration for managing clients, loans, savings, shares, financial transactions and generating financial reports.
source_repo: openMF/mcp-mifosx
source_url: https://github.com/openMF/mcp-mifosx
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
verified_at: 2026-05-26
related: []
tags: [mcp, finance-fintech]
stars: 25
forks: 38
pushed_at: "2026-09-01T21:51:55Z"
---
## What it is
A core banking integration for managing clients, loans, savings, shares, financial transactions and generating financial reports.

## When to use it
When an agent needs the "Finance & Fintech" capability this MCP server exposes.

## Source
Migrated from the awesome-mcp-servers navigation directory (category: Finance & Fintech). See https://github.com/openMF/mcp-mifosx. Pending verify -> promote.
