---
name: greirson-todoist
type: mcps
description: >
  Integrates with Todoist API to manage tasks, projects, sections, and comments with support for bulk operations, natural language search, and comprehensive CRUD functionality.
source_repo: greirson/mcp-todoist
source_url: https://github.com/greirson/mcp-todoist
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 243
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 35
pushed_at: "2026-05-01T10:23:20Z"
---
## What it is
MCP server `Todoist`, catalogued on PulseMCP. Integrates with Todoist API to manage tasks, projects, sections, and comments with support for bulk operations, natural language search, and comprehensive CRUD functionality.

## When to use it
Integrates with Todoist API to manage tasks, projects, sections, and comments with support for bulk operations, natural language search, and comprehensive CRUD functionality.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/greirson/mcp-todoist

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/greirson-todoist). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
