---
name: cuba-exec
type: mcps
description: >
  Advanced shell command execution for AI agents — a Model Context Protocol (MCP) server with security policy engine, process lifecycle management, bounded output capture, POSIX signals, and token-efficient responses.
source_repo: LeandroPG19/cuba-exec
source_url: https://github.com/LeandroPG19/cuba-exec
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 2
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Advanced shell command execution for AI agents — a Model Context Protocol (MCP) server with security policy engine, process lifecycle management, bounded output capture, POSIX signals, and token-efficient responses.

## When to use it
Advanced shell command execution for AI agents — a Model Context Protocol (MCP) server with security policy engine, process lifecycle management, bounded output capture, POSIX signals, and token-efficient responses.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/avwur6lciv) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
