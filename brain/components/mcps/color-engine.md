---
name: color-engine
type: mcps
description: >
  Provides CSS color utilities including parsing, conversion, gamut mapping, WCAG contrast, tint/shade ramp generation, and contrast-target solving.
source_repo: aimsise/color-engine-mcp
source_url: https://github.com/aimsise/color-engine-mcp
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-06-11T01:25:24Z"
---
## What it is
Provides CSS color utilities including parsing, conversion, gamut mapping, WCAG contrast, tint/shade ramp generation, and contrast-target solving.

## When to use it
Provides CSS color utilities including parsing, conversion, gamut mapping, WCAG contrast, tint/shade ramp generation, and contrast-target solving.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/s8jnqctgxn) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
