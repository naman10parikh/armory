---
name: mcp-server-matrix
type: mcps
description: >
  Enables AI assistants to interact with Matrix, the open decentralized communication protocol, allowing them to send and read messages, manage rooms, and perform other Matrix operations through natural language.
source_repo: Baho73/mcp-server-matrix
source_url: https://github.com/Baho73/mcp-server-matrix
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 3
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 1
pushed_at: "2026-04-03T22:30:53Z"
---
## What it is
Enables AI assistants to interact with Matrix, the open decentralized communication protocol, allowing them to send and read messages, manage rooms, and perform other Matrix operations through natural language.

## When to use it
Enables AI assistants to interact with Matrix, the open decentralized communication protocol, allowing them to send and read messages, manage rooms, and perform other Matrix operations through natural language.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/ltapkpe761) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
