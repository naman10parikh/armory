---
name: simplemcpserver
type: mcps
description: >
  Enables AI agents (like Claude Code, Cursor) to directly operate Unity scenes via MCP protocol, with tools for scene hierarchy, object creation/deletion, and transform modification.
source_repo: redcool/SimpleMCPServer
source_url: https://github.com/redcool/SimpleMCPServer
license: ISC License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-08-25T10:15:43Z"
---
## What it is
Enables AI agents (like Claude Code, Cursor) to directly operate Unity scenes via MCP protocol, with tools for scene hierarchy, object creation/deletion, and transform modification.

## When to use it
Enables AI agents (like Claude Code, Cursor) to directly operate Unity scenes via MCP protocol, with tools for scene hierarchy, object creation/deletion, and transform modification.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/hjp1a7pu0k) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
