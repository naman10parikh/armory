---
name: linkedin-evidence-mcp
type: mcps
description: >
  A local-first LinkedIn workspace that turns your own documents, CVs, repositories, and exports into a verifiable claim ledger and evidence-backed canonical profile, enabling evidence import, claim verification, profile patching, post drafting, content planning, analytics, and publication approvals t
source_repo: victoryotzev/linkedin-mcp
source_url: https://github.com/victoryotzev/linkedin-mcp
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
A local-first LinkedIn workspace that turns your own documents, CVs, repositories, and exports into a verifiable claim ledger and evidence-backed canonical profile, enabling evidence import, claim verification, profile patching, post drafting, content planning, analytics, and publication approvals t

## When to use it
A local-first LinkedIn workspace that turns your own documents, CVs, repositories, and exports into a verifiable claim ledger and evidence-backed canonical profile, enabling evidence import, claim verification, profile patching, post drafting, content planning, analytics, and publication approvals t

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/cdhylmgx73) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
