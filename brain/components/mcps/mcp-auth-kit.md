---
name: mcp-auth-kit
type: mcps
description: >
  A production-minded MCP server kit that provides OAuth 2.1/PKCE authentication, rate limiting, scope-gated tools, and two-phase confirmation for building secure MCP servers with custom tools, identity, and storage.
source_repo: bradburch/mcp-auth-kit
source_url: https://github.com/bradburch/mcp-auth-kit
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-08-28T09:55:27Z"
---
## What it is
A production-minded MCP server kit that provides OAuth 2.1/PKCE authentication, rate limiting, scope-gated tools, and two-phase confirmation for building secure MCP servers with custom tools, identity, and storage.

## When to use it
A production-minded MCP server kit that provides OAuth 2.1/PKCE authentication, rate limiting, scope-gated tools, and two-phase confirmation for building secure MCP servers with custom tools, identity, and storage.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/kxd1gh15i3) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
