---
name: incident-io-mcp-server
type: mcps
description: >
  Enables interaction with Incident.io to manage incidents, severities, and health checks via natural language. Provides endpoints for getting and creating incidents.
source_repo: stuartphilp/incident-io-mcp-server
source_url: https://github.com/stuartphilp/incident-io-mcp-server
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 1
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 2
pushed_at: "2025-04-11T21:06:12Z"
---
## What it is
Enables interaction with Incident.io to manage incidents, severities, and health checks via natural language. Provides endpoints for getting and creating incidents.

## When to use it
Enables interaction with Incident.io to manage incidents, severities, and health checks via natural language. Provides endpoints for getting and creating incidents.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/wupdweq3wq) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
