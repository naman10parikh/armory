---
name: genesys-mcp-server
type: mcps
description: >
  A read-only MCP server that exposes Genesys Cloud queue lookup, analytics aggregates, and conversation search tools, enabling natural language queries for contact center metrics.
source_repo: goodellcurtisPFG/genesys-mcp-server
source_url: https://github.com/goodellcurtisPFG/genesys-mcp-server
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
A read-only MCP server that exposes Genesys Cloud queue lookup, analytics aggregates, and conversation search tools, enabling natural language queries for contact center metrics.

## When to use it
A read-only MCP server that exposes Genesys Cloud queue lookup, analytics aggregates, and conversation search tools, enabling natural language queries for contact center metrics.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/z379jizbga) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
