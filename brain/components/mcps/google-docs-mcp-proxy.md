---
name: google-docs-mcp-proxy
type: mcps
description: >
  Local stdio MCP server that handles Google OAuth2 locally and proxies authenticated requests to Google Docs and Google Drive REST APIs, enabling document and file operations from Cursor or other MCP clients.
source_repo: dsingh80/google-docs-mcp
source_url: https://github.com/dsingh80/google-docs-mcp
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Local stdio MCP server that handles Google OAuth2 locally and proxies authenticated requests to Google Docs and Google Drive REST APIs, enabling document and file operations from Cursor or other MCP clients.

## When to use it
Local stdio MCP server that handles Google OAuth2 locally and proxies authenticated requests to Google Docs and Google Drive REST APIs, enabling document and file operations from Cursor or other MCP clients.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/yp5f84wcxt) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
