---
name: memory
type: mcps
description: >
  A basic implementation of persistent memory using a local knowledge graph. This lets Claude remember information about the user across chats.
source_repo: modelcontextprotocol/servers
source_url: https://github.com/modelcontextprotocol/servers/blob/main/src/memory/README.md
license: Apache-2.0
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: stable
stars: null
eval_score: null
verified_at: 2026-05-26
related: []
tags: [mcp, official, reference, mcp-server]
mentions: null
---
## What it is
Official `modelcontextprotocol/servers` mcps component — A basic implementation of persistent memory using a local knowledge graph. This lets Claude remember information about the user across chats.

## When to use it
A basic implementation of persistent memory using a local knowledge graph. This lets Claude remember information about the user across chats.

## How to install / invoke
Official MCP reference server. See the README for the `mcpServers` config block (command + args / npx / uvx): https://github.com/modelcontextprotocol/servers/blob/main/src/memory/README.md

## Notes
Official Model Context Protocol reference server (modelcontextprotocol/servers). License: Apache-2.0. Pending verify → promote.
