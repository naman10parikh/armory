---
name: http-client-mcp-server
type: mcps
description: >
  Enables AI assistants to explore, search, and read codebase repositories and API specifications efficiently, with support for file searching, content search via ripgrep, and reading API specs.
source_repo: noxhsxrk/http-client-mcp-server
source_url: https://github.com/noxhsxrk/http-client-mcp-server
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-04-08T09:23:26Z"
---
## What it is
Enables AI assistants to explore, search, and read codebase repositories and API specifications efficiently, with support for file searching, content search via ripgrep, and reading API specs.

## When to use it
Enables AI assistants to explore, search, and read codebase repositories and API specifications efficiently, with support for file searching, content search via ripgrep, and reading API specs.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/ua4p8wdh6n) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
