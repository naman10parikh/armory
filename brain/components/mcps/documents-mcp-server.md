---
name: documents-mcp-server
type: mcps
description: >
  A production-ready Model Context Protocol server that bridges local document management with cloud synchronization (Notion) for AI agent integration, enabling seamless access and sync of local and cloud documents.
source_repo: corneyc/documents-mcp
source_url: https://github.com/corneyc/documents-mcp
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
A production-ready Model Context Protocol server that bridges local document management with cloud synchronization (Notion) for AI agent integration, enabling seamless access and sync of local and cloud documents.

## When to use it
A production-ready Model Context Protocol server that bridges local document management with cloud synchronization (Notion) for AI agent integration, enabling seamless access and sync of local and cloud documents.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/nssyjsl7je) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
