---
name: praetor-mcp
type: mcps
description: >
  Provides a local synthetic MCP stdio service for querying maintenance evidence and preparing human-review-gated advisory packets, with deterministic governance, evidence boundary tracing, and Agent K integrity scoring to separate retrieved evidence from model inference while preserving human review
source_repo: drosadocastro-bit/PRAETOR_MCP_BICAMERAL
source_url: https://github.com/drosadocastro-bit/PRAETOR_MCP_BICAMERAL
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Provides a local synthetic MCP stdio service for querying maintenance evidence and preparing human-review-gated advisory packets, with deterministic governance, evidence boundary tracing, and Agent K integrity scoring to separate retrieved evidence from model inference while preserving human review 

## When to use it
Provides a local synthetic MCP stdio service for querying maintenance evidence and preparing human-review-gated advisory packets, with deterministic governance, evidence boundary tracing, and Agent K integrity scoring to separate retrieved evidence from model inference while preserving human review 

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/bxbb29i9b9) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
