---
name: parallel-web-search-mcp
type: mcps
description: >
  🔎 - Highest Accuracy Web Search for AI
source_repo: parallel-web/search-mcp
source_url: https://github.com/parallel-web/search-mcp
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
verified_at: 2026-05-26
related: []
tags: [mcp, search-data-extraction]
stars: null
---
## What it is
🔎 - Highest Accuracy Web Search for AI

## When to use it
When an agent needs the "Search & Data Extraction" capability this MCP server exposes.

## Source
Migrated from the awesome-mcp-servers navigation directory (category: Search & Data Extraction). See https://github.com/parallel-web/search-mcp. Pending verify -> promote.
