---
name: claude-context-server
type: mcps
description: >
  An MCP server that reads all your Claude Code project memory files and exposes them as tools. Lets any Claude instance — in any project, or via Claude.ai — query your full project history and preferences.
source_repo: carlformigoni/claude-context-server
source_url: https://github.com/carlformigoni/claude-context-server
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 1
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-06-14T05:42:13Z"
---
## What it is
An MCP server that reads all your Claude Code project memory files and exposes them as tools. Lets any Claude instance — in any project, or via Claude.ai — query your full project history and preferences.

## When to use it
An MCP server that reads all your Claude Code project memory files and exposes them as tools. Lets any Claude instance — in any project, or via Claude.ai — query your full project history and preferences.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/aug2h1r4pe) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
