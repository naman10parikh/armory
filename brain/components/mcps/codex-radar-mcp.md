---
name: codex-radar-mcp
type: mcps
description: >
  This MCP server exposes a set of tools that fetch real-time model IQ and recommendation data from codexradar.com, allowing MCP clients like MaiBot or Claude Desktop to answer questions about which AI model is currently the smartest, how to configure effort tiers for different usage scenarios, and wh
source_repo: MorphieEndless/codex-radar-mcp
source_url: https://github.com/MorphieEndless/codex-radar-mcp
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
This MCP server exposes a set of tools that fetch real-time model IQ and recommendation data from codexradar.com, allowing MCP clients like MaiBot or Claude Desktop to answer questions about which AI model is currently the smartest, how to configure effort tiers for different usage scenarios, and wh

## When to use it
This MCP server exposes a set of tools that fetch real-time model IQ and recommendation data from codexradar.com, allowing MCP clients like MaiBot or Claude Desktop to answer questions about which AI model is currently the smartest, how to configure effort tiers for different usage scenarios, and wh

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/ap8ejqnvyg) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
