---
name: charmhealth-mcp-server
type: mcps
description: >
  An MCP server for CharmHealth EHR that allows LLMs and MCP clients to interact with patient records, encounters, and practice information.
source_repo: CharmHealth/charm-mcp-server
source_url: https://github.com/CharmHealth/charm-mcp-server
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 10
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 9
pushed_at: "2026-08-28T06:23:34Z"
---
## What it is
An MCP server for CharmHealth EHR that allows LLMs and MCP clients to interact with patient records, encounters, and practice information.

## When to use it
An MCP server for CharmHealth EHR that allows LLMs and MCP clients to interact with patient records, encounters, and practice information.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/l5aub0cdee) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
