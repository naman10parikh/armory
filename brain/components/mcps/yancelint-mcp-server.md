---
name: yancelint-mcp-server
type: mcps
description: >
  Enables AI coding assistants like Claude Code to perform real-time code linting and get violation summaries from tools such as ESLint, Stylelint, P3C, and Checkstyle via a local server.
source_repo: xihe-lab/yance-mcp-server
source_url: https://github.com/xihe-lab/yance-mcp-server
license: Apache 2.0
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-04-21T13:54:35Z"
---
## What it is
Enables AI coding assistants like Claude Code to perform real-time code linting and get violation summaries from tools such as ESLint, Stylelint, P3C, and Checkstyle via a local server.

## When to use it
Enables AI coding assistants like Claude Code to perform real-time code linting and get violation summaries from tools such as ESLint, Stylelint, P3C, and Checkstyle via a local server.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/pmotkbbtpj) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
