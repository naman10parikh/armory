---
name: pptxgenjs-mcp-server
type: mcps
description: >
  A production-ready MCP server that enables LLMs to create, modify, and export PowerPoint presentations programmatically using PptxGenJS.
source_repo: Hrithik-s-Raj/PptxGenJS-mcp-server
source_url: https://github.com/Hrithik-s-Raj/PptxGenJS-mcp-server
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 1
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 2
pushed_at: "2025-10-24T14:21:39Z"
---
## What it is
A production-ready MCP server that enables LLMs to create, modify, and export PowerPoint presentations programmatically using PptxGenJS.

## When to use it
A production-ready MCP server that enables LLMs to create, modify, and export PowerPoint presentations programmatically using PptxGenJS.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/dckgdjx5sy) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
