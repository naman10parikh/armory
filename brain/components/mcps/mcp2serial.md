---
name: mcp2serial
type: mcps
description: >
  Bridges with physical hardware devices (e.g. Raspberry Pi) via serial communication, enabling real-world control and interaction for IoT and robotics applications.
source_repo: mcp2everything/mcp2serial
source_url: https://github.com/mcp2everything/mcp2serial
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 49
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 12
pushed_at: "2024-12-19T08:49:31Z"
---
## What it is
MCP server `Serial`, catalogued on PulseMCP. Bridges with physical hardware devices (e.g. Raspberry Pi) via serial communication, enabling real-world control and interaction for IoT and robotics applications.

## When to use it
Bridges with physical hardware devices (e.g. Raspberry Pi) via serial communication, enabling real-world control and interaction for IoT and robotics applications.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/mcp2everything/mcp2serial

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/mcp2serial). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
