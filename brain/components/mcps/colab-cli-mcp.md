---
name: colab-cli-mcp
type: mcps
description: >
  Enables MCP clients to execute Python code on Google Colab GPU/TPU runtimes via the official Colab CLI, with session, file, and package management capabilities.
source_repo: animationsaver/colab-cli-mcp
source_url: https://github.com/animationsaver/colab-cli-mcp
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-08-09T17:04:59Z"
---
## What it is
Enables MCP clients to execute Python code on Google Colab GPU/TPU runtimes via the official Colab CLI, with session, file, and package management capabilities.

## When to use it
Enables MCP clients to execute Python code on Google Colab GPU/TPU runtimes via the official Colab CLI, with session, file, and package management capabilities.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/gymri0j1y6) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
