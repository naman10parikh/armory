---
name: ldap-assistant-mcp
type: mcps
description: >
  Enables troubleshooting of LDAP directory services through natural language queries, providing health diagnostics, replication status, performance metrics, log analysis, and configuration comparisons across multiple servers.
source_repo: droideck/ldap-assistant-mcp
source_url: https://github.com/droideck/ldap-assistant-mcp
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Enables troubleshooting of LDAP directory services through natural language queries, providing health diagnostics, replication status, performance metrics, log analysis, and configuration comparisons across multiple servers.

## When to use it
Enables troubleshooting of LDAP directory services through natural language queries, providing health diagnostics, replication status, performance metrics, log analysis, and configuration comparisons across multiple servers.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/swu0ujwcbf) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
