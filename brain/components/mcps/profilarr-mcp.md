---
name: profilarr-mcp
type: mcps
description: >
  MCP server exposing Profilarr's v1 REST API as tools, enabling an LLM to inspect and manage linked databases, connected Radarr/Sonarr instances, backups, jobs, announcements, and system status.
source_repo: SavageCore/profilarr-mcp
source_url: https://github.com/SavageCore/profilarr-mcp
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
MCP server exposing Profilarr's v1 REST API as tools, enabling an LLM to inspect and manage linked databases, connected Radarr/Sonarr instances, backups, jobs, announcements, and system status.

## When to use it
MCP server exposing Profilarr's v1 REST API as tools, enabling an LLM to inspect and manage linked databases, connected Radarr/Sonarr instances, backups, jobs, announcements, and system status.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/bbw5lzxkzg) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
