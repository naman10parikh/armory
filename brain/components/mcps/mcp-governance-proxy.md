---
name: mcp-governance-proxy
type: mcps
description: >
  An MCP server that acts as a governance proxy for AI agents, evaluating each tool call against policies before execution, enabling secure and controlled access to systems like Slack, GitHub, and AWS without exposing credentials to the agent.
source_repo: andreas-altamirano/mcp-governance-proxy
source_url: https://github.com/andreas-altamirano/mcp-governance-proxy
license: Apache 2.0
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
An MCP server that acts as a governance proxy for AI agents, evaluating each tool call against policies before execution, enabling secure and controlled access to systems like Slack, GitHub, and AWS without exposing credentials to the agent.

## When to use it
An MCP server that acts as a governance proxy for AI agents, evaluating each tool call against policies before execution, enabling secure and controlled access to systems like Slack, GitHub, and AWS without exposing credentials to the agent.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/r6x0nfq7m9) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
