---
name: xuanwo-opendal
type: mcps
description: >
  Integrates with OpenDAL to provide unified access to diverse storage backends, enabling LLMs to read from and write to various storage systems for data management tasks.
source_repo: xuanwo/mcp-server-opendal
source_url: https://github.com/xuanwo/mcp-server-opendal
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 35
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 10
pushed_at: "2025-04-10T09:03:26Z"
---
## What it is
MCP server `OpenDAL`, catalogued on PulseMCP. Integrates with OpenDAL to provide unified access to diverse storage backends, enabling LLMs to read from and write to various storage systems for data management tasks.

## When to use it
Integrates with OpenDAL to provide unified access to diverse storage backends, enabling LLMs to read from and write to various storage systems for data management tasks.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/xuanwo/mcp-server-opendal

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/xuanwo-opendal). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
