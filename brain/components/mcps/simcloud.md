---
name: simcloud
type: mcps
description: >
  SIMcloud is a hosted South African MCP server for wallet balance checks, network lookups, SMS sending, airtime recharges, data bundle purchases, and prepaid electricity orders. It connects over Streamable HTTP and uses each user's own SIMcloud account via OAuth.
source_repo: ErikThiart/simcloud-codex-plugin
source_url: https://github.com/ErikThiart/simcloud-codex-plugin
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
SIMcloud is a hosted South African MCP server for wallet balance checks, network lookups, SMS sending, airtime recharges, data bundle purchases, and prepaid electricity orders. It connects over Streamable HTTP and uses each user's own SIMcloud account via OAuth.

## When to use it
SIMcloud is a hosted South African MCP server for wallet balance checks, network lookups, SMS sending, airtime recharges, data bundle purchases, and prepaid electricity orders. It connects over Streamable HTTP and uses each user's own SIMcloud account via OAuth.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/p3bzyyx3kr) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
