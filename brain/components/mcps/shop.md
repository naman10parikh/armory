---
name: shop
type: mcps
description: >
  Enables read-only interaction with an online store's SQLite database over MCP stdio, including table listing, schema inspection, safe read-only SQL execution, and sales analytics. It rejects mutating SQL operations to keep data intact.
source_repo: DrZeD-13/test-mcp-db
source_url: https://github.com/DrZeD-13/test-mcp-db
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Enables read-only interaction with an online store's SQLite database over MCP stdio, including table listing, schema inspection, safe read-only SQL execution, and sales analytics. It rejects mutating SQL operations to keep data intact.

## When to use it
Enables read-only interaction with an online store's SQLite database over MCP stdio, including table listing, schema inspection, safe read-only SQL execution, and sales analytics. It rejects mutating SQL operations to keep data intact.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/cn8kciidm1) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
