---
name: 2ools-mcp-server
type: mcps
description: >
  Remote MCP server for 2ools—build, version, review, and export websites, web apps, and games from the AI chat you already use. This repository includes a credential-free stdio proxy and Dockerfile that forward introspection to the verified remote endpoint without mocking tools.
source_repo: 2ools/mcp-server
source_url: https://github.com/2ools/mcp-server
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Remote MCP server for 2ools—build, version, review, and export websites, web apps, and games from the AI chat you already use. This repository includes a credential-free stdio proxy and Dockerfile that forward introspection to the verified remote endpoint without mocking tools.

## When to use it
Remote MCP server for 2ools—build, version, review, and export websites, web apps, and games from the AI chat you already use. This repository includes a credential-free stdio proxy and Dockerfile that forward introspection to the verified remote endpoint without mocking tools.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/ywifyp8zj2) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
