---
name: facebook-mcp-server
type: mcps
description: >
  A Model Context Protocol (MCP) server implementation that provides Facebook Page interaction and management capabilities. This server enables automated posting, comment moderation, and content retrieval.
source_repo: tiroshanm/facebook-mcp-server
source_url: https://github.com/tiroshanm/facebook-mcp-server
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 75
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 26
pushed_at: "2025-04-14T06:50:48Z"
---
## What it is
A Model Context Protocol (MCP) server implementation that provides Facebook Page interaction and management capabilities. This server enables automated posting, comment moderation, and content retrieval.

## When to use it
A Model Context Protocol (MCP) server implementation that provides Facebook Page interaction and management capabilities. This server enables automated posting, comment moderation, and content retrieval.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/rkjg0kmml9) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
