---
name: elsewhere
type: mcps
description: >
  MCP server that provides coding agents with scoped, temporary network egress sessions, enabling requests to exit from specified locations without changing the host's connection. It verifies egress, prevents leaks, and supports multiple adapters like gluetun and SSH.
source_repo: noelbraganza/elsewhere
source_url: https://github.com/noelbraganza/elsewhere
license: Apache 2.0
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
MCP server that provides coding agents with scoped, temporary network egress sessions, enabling requests to exit from specified locations without changing the host's connection. It verifies egress, prevents leaks, and supports multiple adapters like gluetun and SSH.

## When to use it
MCP server that provides coding agents with scoped, temporary network egress sessions, enabling requests to exit from specified locations without changing the host's connection. It verifies egress, prevents leaks, and supports multiple adapters like gluetun and SSH.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/nkfexauvaq) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
