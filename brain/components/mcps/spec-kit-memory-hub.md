---
name: spec-kit-memory-hub
type: mcps
description: >
  Repository-native, Git-reviewable memory extension for AI coding assistants to persist project context across features, using SQLite caching for up to 10x token reduction.
source_repo: DyanGalih/spec-kit-memory-hub
source_url: https://github.com/DyanGalih/spec-kit-memory-hub
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 15
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 1
pushed_at: "2026-05-23T00:07:55Z"
---
## What it is
Repository-native, Git-reviewable memory extension for AI coding assistants to persist project context across features, using SQLite caching for up to 10x token reduction.

## When to use it
Repository-native, Git-reviewable memory extension for AI coding assistants to persist project context across features, using SQLite caching for up to 10x token reduction.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/cpl635mfl2) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
