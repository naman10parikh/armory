---
name: lpbayliss-dice-roll
type: mcps
description: >
  Provides cryptographically secure dice rolling with standard notation and Fate/Fudge dice support for tabletop RPGs and games requiring reliable random number generation.
source_repo: lpbayliss/server-dice-roll
source_url: https://github.com/lpbayliss/server-dice-roll
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 2
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 1
pushed_at: "2026-08-29T22:47:40Z"
---
## What it is
MCP server `Dice Roller`, catalogued on PulseMCP. Provides cryptographically secure dice rolling with standard notation and Fate/Fudge dice support for tabletop RPGs and games requiring reliable random number generation.

## When to use it
Provides cryptographically secure dice rolling with standard notation and Fate/Fudge dice support for tabletop RPGs and games requiring reliable random number generation.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/lpbayliss/server-dice-roll

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/lpbayliss-dice-roll). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
