---
name: openfin-mcp
type: mcps
description: >
  Provides read-only access to curated financial domain knowledge and product data with verified provenance, enabling users to search and retrieve trusted financial information.
source_repo: jhny-kor/OpenFin
source_url: https://github.com/jhny-kor/OpenFin
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-09-01T13:24:50Z"
---
## What it is
Provides read-only access to curated financial domain knowledge and product data with verified provenance, enabling users to search and retrieve trusted financial information.

## When to use it
Provides read-only access to curated financial domain knowledge and product data with verified provenance, enabling users to search and retrieve trusted financial information.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/ylxhtd3ube) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
