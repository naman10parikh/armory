---
name: hive-intel-hive-crypto-mcp
type: mcps
description: >
  Hive Intelligence: Ultimate cryptocurrency MCP for AI assistants with unified access to crypto, DeFi, and Web3 analytics
source_repo: hive-intel/hive-crypto-mcp
source_url: https://github.com/hive-intel/hive-crypto-mcp
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
verified_at: 2026-05-26
related: []
tags: [mcp, finance-fintech]
stars: 18
forks: 6
pushed_at: "2026-08-30T19:03:49Z"
---
## What it is
Hive Intelligence: Ultimate cryptocurrency MCP for AI assistants with unified access to crypto, DeFi, and Web3 analytics

## When to use it
When an agent needs the "Finance & Fintech" capability this MCP server exposes.

## Source
Migrated from the awesome-mcp-servers navigation directory (category: Finance & Fintech). See https://github.com/hive-intel/hive-crypto-mcp. Pending verify -> promote.
