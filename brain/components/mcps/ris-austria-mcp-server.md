---
name: ris-austria-mcp-server
type: mcps
description: >
  Search Austrian consolidated federal & state law, court decisions, and the authentic Bundesgesetzblatt from the official RIS via MCP, keyless and supporting STDIO or Streamable HTTP.
source_repo: cyanheads/ris-austria-mcp-server
source_url: https://github.com/cyanheads/ris-austria-mcp-server
license: Apache 2.0
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 2
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Search Austrian consolidated federal & state law, court decisions, and the authentic Bundesgesetzblatt from the official RIS via MCP, keyless and supporting STDIO or Streamable HTTP.

## When to use it
Search Austrian consolidated federal & state law, court decisions, and the authentic Bundesgesetzblatt from the official RIS via MCP, keyless and supporting STDIO or Streamable HTTP.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/rgyrhwd0ky) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
