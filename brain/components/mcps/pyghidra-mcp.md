---
name: pyghidra-mcp
type: mcps
description: >
  Exposes Ghidra reverse engineering capabilities via MCP, enabling LLMs and agents to analyze binaries, decompile, search, and edit programs headlessly or with GUI integration.
source_repo: clearbluejar/pyghidra-mcp
source_url: https://github.com/clearbluejar/pyghidra-mcp
license: Apache 2.0
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 413
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Exposes Ghidra reverse engineering capabilities via MCP, enabling LLMs and agents to analyze binaries, decompile, search, and edit programs headlessly or with GUI integration.

## When to use it
Exposes Ghidra reverse engineering capabilities via MCP, enabling LLMs and agents to analyze binaries, decompile, search, and edit programs headlessly or with GUI integration.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/nss6sffyni) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
