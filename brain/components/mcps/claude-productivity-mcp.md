---
name: claude-productivity-mcp
type: mcps
description: >
  An MCP server that integrates popular productivity tools like Gmail, Google Calendar, Google Drive, Google Sheets, Notion, and Microsoft Outlook, allowing Claude Desktop to interact with them via natural language commands.
source_repo: kmusicman/claude-productivity-mcp
source_url: https://github.com/kmusicman/claude-productivity-mcp
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
An MCP server that integrates popular productivity tools like Gmail, Google Calendar, Google Drive, Google Sheets, Notion, and Microsoft Outlook, allowing Claude Desktop to interact with them via natural language commands.

## When to use it
An MCP server that integrates popular productivity tools like Gmail, Google Calendar, Google Drive, Google Sheets, Notion, and Microsoft Outlook, allowing Claude Desktop to interact with them via natural language commands.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/n3ctmugv5e) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
