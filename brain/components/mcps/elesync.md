---
name: elesync
type: mcps
description: >
  Privacy-first local memory vault every AI shares over MCP. Markdown + SQLite on your machine; Claude, ChatGPT, Cursor, and any MCP client read and write it live. No cloud, no account, no telemetry
source_repo: darknodebros/EleSync
source_url: https://github.com/darknodebros/EleSync
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Privacy-first local memory vault every AI shares over MCP. Markdown + SQLite on your machine; Claude, ChatGPT, Cursor, and any MCP client read and write it live. No cloud, no account, no telemetry

## When to use it
Privacy-first local memory vault every AI shares over MCP. Markdown + SQLite on your machine; Claude, ChatGPT, Cursor, and any MCP client read and write it live. No cloud, no account, no telemetry

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/rnljzw8bft) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
