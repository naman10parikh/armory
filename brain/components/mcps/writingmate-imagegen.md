---
name: writingmate-imagegen
type: mcps
description: >
  Provides image generation across multiple AI providers (OpenAI DALL-E, Google Gemini, Replicate Flux) with unified parameter handling, automatic file saving, and provider-specific features like transparent backgrounds and seed control for flexible visual content creation.
source_repo: writingmate/imagegen-mcp
source_url: https://github.com/writingmate/imagegen-mcp
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 10
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 5
pushed_at: "2025-09-09T17:39:07Z"
---
## What it is
MCP server `ImageGen`, catalogued on PulseMCP. Provides image generation across multiple AI providers (OpenAI DALL-E, Google Gemini, Replicate Flux) with unified parameter handling, automatic file saving, and provider-specific features like transparent backgrounds and seed control for flexible visual content creation.

## When to use it
Provides image generation across multiple AI providers (OpenAI DALL-E, Google Gemini, Replicate Flux) with unified parameter handling, automatic file saving, and provider-specific features like transparent backgrounds and seed control for flexible visual content creation.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/writingmate/imagegen-mcp

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/writingmate-imagegen). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
