---
name: narumiruna-yahoo-finance
type: mcps
description: >
  Provides real-time financial data from Yahoo Finance through specialized tools for retrieving stock information, market trends, and news for investment research and analysis.
source_repo: narumiruna/yfinance-mcp
source_url: https://github.com/narumiruna/yfinance-mcp
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 188
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 66
pushed_at: "2026-08-30T14:54:02Z"
---
## What it is
MCP server `Yahoo Finance`, catalogued on PulseMCP. Provides real-time financial data from Yahoo Finance through specialized tools for retrieving stock information, market trends, and news for investment research and analysis.

## When to use it
Provides real-time financial data from Yahoo Finance through specialized tools for retrieving stock information, market trends, and news for investment research and analysis.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/narumiruna/yfinance-mcp

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/narumiruna-yahoo-finance). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
