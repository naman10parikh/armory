---
name: buryhuang-unipile
type: mcps
description: >
  Integrates with Unipile to provide unified access to messages across LinkedIn, WhatsApp, Instagram, Messenger, and Telegram, enabling efficient multi-platform communication management.
source_repo: honeybluesky/mcp-unipile
source_url: https://github.com/honeybluesky/mcp-unipile
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 16
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 11
pushed_at: "2025-01-07T07:39:31Z"
---
## What it is
MCP server `Unipile`, catalogued on PulseMCP. Integrates with Unipile to provide unified access to messages across LinkedIn, WhatsApp, Instagram, Messenger, and Telegram, enabling efficient multi-platform communication management.

## When to use it
Integrates with Unipile to provide unified access to messages across LinkedIn, WhatsApp, Instagram, Messenger, and Telegram, enabling efficient multi-platform communication management.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/honeybluesky/mcp-unipile

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/buryhuang-unipile). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
