---
name: bukio-cli
type: mcps
description: >
  MCP server for agent-first double-entry bookkeeping for Dutch SMEs, supporting VAT, Peppol BIS 3.0 e-invoicing, and local-first SQLite storage with full audit logging and deterministic JSON output.
source_repo: erikvankempen/bukio-cli
source_url: https://github.com/erikvankempen/bukio-cli
license: Apache 2.0
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
MCP server for agent-first double-entry bookkeeping for Dutch SMEs, supporting VAT, Peppol BIS 3.0 e-invoicing, and local-first SQLite storage with full audit logging and deterministic JSON output.

## When to use it
MCP server for agent-first double-entry bookkeeping for Dutch SMEs, supporting VAT, Peppol BIS 3.0 e-invoicing, and local-first SQLite storage with full audit logging and deterministic JSON output.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/e3yhmpoq4f) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
