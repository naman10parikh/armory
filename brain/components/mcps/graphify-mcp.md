---
name: graphify-mcp
type: mcps
description: >
  A Python MCP server that exposes the Graphify knowledge graph as MCP tools, prompts, and resources, enabling AI assistants to explore codebases through a token-budgeted, structural graph during development.
source_repo: yasinyaman/graphify-mcp
source_url: https://github.com/yasinyaman/graphify-mcp
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 2
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-08-30T13:13:49Z"
---
## What it is
A Python MCP server that exposes the Graphify knowledge graph as MCP tools, prompts, and resources, enabling AI assistants to explore codebases through a token-budgeted, structural graph during development.

## When to use it
A Python MCP server that exposes the Graphify knowledge graph as MCP tools, prompts, and resources, enabling AI assistants to explore codebases through a token-budgeted, structural graph during development.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/mvtepbplwn) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
