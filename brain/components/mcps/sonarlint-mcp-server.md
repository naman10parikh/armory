---
name: sonarlint-mcp-server
type: mcps
description: >
  Enables real-time code analysis for JavaScript, TypeScript, and Python through Claude Desktop and other MCP clients, detecting bugs, code smells, and security vulnerabilities with automated quick fixes.
source_repo: nielspeter/sonarlint-mcp-server
source_url: https://github.com/nielspeter/sonarlint-mcp-server
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 4
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Enables real-time code analysis for JavaScript, TypeScript, and Python through Claude Desktop and other MCP clients, detecting bugs, code smells, and security vulnerabilities with automated quick fixes.

## When to use it
Enables real-time code analysis for JavaScript, TypeScript, and Python through Claude Desktop and other MCP clients, detecting bugs, code smells, and security vulnerabilities with automated quick fixes.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/gip91b1vq7) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
