---
name: boxadm-mcp
type: mcps
description: >
  Read-only MCP server that surfaces external file sharing risks from Box enterprise event logs, enabling early-warning leakage detection without modifying any data.
source_repo: shigechika/boxadm-mcp
source_url: https://github.com/shigechika/boxadm-mcp
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Read-only MCP server that surfaces external file sharing risks from Box enterprise event logs, enabling early-warning leakage detection without modifying any data.

## When to use it
Read-only MCP server that surfaces external file sharing risks from Box enterprise event logs, enabling early-warning leakage detection without modifying any data.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/offomox2aw) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
