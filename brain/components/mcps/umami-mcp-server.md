---
name: umami-mcp-server
type: mcps
description: >
  An MCP server that gives Claude read access to Umami web analytics, allowing natural language queries for stats, breakdowns, pageview trends, live visitors, and user journeys.
source_repo: MurkyPuma/umami-mcp-server
source_url: https://github.com/MurkyPuma/umami-mcp-server
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 1
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-07-20T17:57:40Z"
---
## What it is
An MCP server that gives Claude read access to Umami web analytics, allowing natural language queries for stats, breakdowns, pageview trends, live visitors, and user journeys.

## When to use it
An MCP server that gives Claude read access to Umami web analytics, allowing natural language queries for stats, breakdowns, pageview trends, live visitors, and user journeys.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/nixvqzjf46) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
