---
name: opencode-web-audit-mcp
type: mcps
description: >
  A read-only Python MCP server for authorized website research, structured data extraction, downloadable-document analysis, and content auditing inside OpenCode. It crawls authorized public domains with safety constraints including robots.txt respect, SSRF defenses, bounded concurrency, and content-t
source_repo: humbertolvarona/opencode-web-audit-mcp-python
source_url: https://github.com/humbertolvarona/opencode-web-audit-mcp-python
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
A read-only Python MCP server for authorized website research, structured data extraction, downloadable-document analysis, and content auditing inside OpenCode. It crawls authorized public domains with safety constraints including robots.txt respect, SSRF defenses, bounded concurrency, and content-t

## When to use it
A read-only Python MCP server for authorized website research, structured data extraction, downloadable-document analysis, and content auditing inside OpenCode. It crawls authorized public domains with safety constraints including robots.txt respect, SSRF defenses, bounded concurrency, and content-t

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/p811pavnwf) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
