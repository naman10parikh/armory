---
name: tracebug
type: mcps
description: >
  Local stdio MCP server that gives AI coding agents (Claude Code, Cursor, Windsurf) read-tools over TraceBug bug-report .html files — session replay, console errors with stacks, network activity, and a millisecond repro timeline. All data is read from local disk; zero network connections. Free, MIT.
source_repo: prashantsinghmangat/TraceBug-ai
source_url: https://github.com/prashantsinghmangat/TraceBug-ai
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Local stdio MCP server that gives AI coding agents (Claude Code, Cursor, Windsurf) read-tools over TraceBug bug-report .html files — session replay, console errors with stacks, network activity, and a millisecond repro timeline. All data is read from local disk; zero network connections. Free, MIT. 

## When to use it
Local stdio MCP server that gives AI coding agents (Claude Code, Cursor, Windsurf) read-tools over TraceBug bug-report .html files — session replay, console errors with stacks, network activity, and a millisecond repro timeline. All data is read from local disk; zero network connections. Free, MIT. 

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/w4u0uzrb07) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
