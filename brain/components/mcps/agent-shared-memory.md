---
name: agent-shared-memory
type: mcps
description: >
  Enables AI agents to share persistent memory via an MCP server using SQLite, supporting multi-tenant, categorized knowledge with TTL and semantic links, without requiring vector databases.
source_repo: CidLucas/agent-shared-memory
source_url: https://github.com/CidLucas/agent-shared-memory
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Enables AI agents to share persistent memory via an MCP server using SQLite, supporting multi-tenant, categorized knowledge with TTL and semantic links, without requiring vector databases.

## When to use it
Enables AI agents to share persistent memory via an MCP server using SQLite, supporting multi-tenant, categorized knowledge with TTL and semantic links, without requiring vector databases.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/xevv75j3eh) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
