---
name: wiki-loop-mcp
type: mcps
description: >
  MCP server that provides tools to add to, search, and manage a wiki knowledge base, enabling AI chat tools to contribute and retrieve information from the wiki via natural language prompts.
source_repo: Prometheus-AGS/prometheus-wiki-loop
source_url: https://github.com/Prometheus-AGS/prometheus-wiki-loop
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-06-30T21:58:10Z"
---
## What it is
MCP server that provides tools to add to, search, and manage a wiki knowledge base, enabling AI chat tools to contribute and retrieve information from the wiki via natural language prompts.

## When to use it
MCP server that provides tools to add to, search, and manage a wiki knowledge base, enabling AI chat tools to contribute and retrieve information from the wiki via natural language prompts.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/nacv5boml4) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
