---
name: zaizaizhao-mcp-swagger-server
type: mcps
description: >
  A Model Context Protocol (MCP) server that converts OpenAPI/Swagger specifications to MCP format, enabling AI assistants to interact with REST APIs through standardized protocol.
source_repo: zaizaizhao/mcp-swagger-server
source_url: https://github.com/zaizaizhao/mcp-swagger-server
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
verified_at: 2026-05-26
related: []
tags: [mcp, developer-tools]
stars: 74
forks: 16
pushed_at: "2026-08-12T01:35:35Z"
---
## What it is
A Model Context Protocol (MCP) server that converts OpenAPI/Swagger specifications to MCP format, enabling AI assistants to interact with REST APIs through standardized protocol.

## When to use it
When an agent needs the "Developer Tools" capability this MCP server exposes.

## Source
Migrated from the awesome-mcp-servers navigation directory (category: Developer Tools). See https://github.com/zaizaizhao/mcp-swagger-server. Pending verify -> promote.
