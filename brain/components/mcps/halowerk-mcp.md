---
name: halowerk-mcp
type: mcps
description: >
  Provides access to 70 paid APIs as MCP tools with per-call and session budgets, paying via x402/USDC on Base without requiring an account or login. Includes a built-in allowlist of hosts and safe defaults to prevent unintended payments.
source_repo: halowerk/halowerk-mcp
source_url: https://github.com/halowerk/halowerk-mcp
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-08-12T09:06:13Z"
---
## What it is
Provides access to 70 paid APIs as MCP tools with per-call and session budgets, paying via x402/USDC on Base without requiring an account or login. Includes a built-in allowlist of hosts and safe defaults to prevent unintended payments.

## When to use it
Provides access to 70 paid APIs as MCP tools with per-call and session budgets, paying via x402/USDC on Base without requiring an account or login. Includes a built-in allowlist of hosts and safe defaults to prevent unintended payments.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/vnn9piw8be) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
