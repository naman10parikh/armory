---
name: priyankark-a11y
type: mcps
description: >
  Perform accessibility audits on webpages using axe-core. Use the results in an agentic loop with your favorite AI assistants (Cline/Cursor/GH Copilot) and let them fix a11y issues for you.
source_repo: priyankark/a11y-mcp
source_url: https://github.com/priyankark/a11y-mcp
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 51
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 9
pushed_at: "2026-03-22T15:23:46Z"
---
## What it is
MCP server `A11y`, catalogued on PulseMCP. Perform accessibility audits on webpages using axe-core. Use the results in an agentic loop with your favorite AI assistants (Cline/Cursor/GH Copilot) and let them fix a11y issues for you.

## When to use it
Perform accessibility audits on webpages using axe-core. Use the results in an agentic loop with your favorite AI assistants (Cline/Cursor/GH Copilot) and let them fix a11y issues for you.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/priyankark/a11y-mcp

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/priyankark-a11y). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
