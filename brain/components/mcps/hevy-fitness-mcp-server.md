---
name: hevy-fitness-mcp-server
type: mcps
description: >
  A Model Context Protocol (MCP) server that provides AI assistants with access to the Hevy fitness tracking API. This allows you to log workouts, manage routines, browse exercises, and track your fitness progress directly through AI chat interfaces.
source_repo: phaiel/hevy-mcp-server
source_url: https://github.com/phaiel/hevy-mcp-server
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-07-06T15:40:12Z"
---
## What it is
A Model Context Protocol (MCP) server that provides AI assistants with access to the Hevy fitness tracking API. This allows you to log workouts, manage routines, browse exercises, and track your fitness progress directly through AI chat interfaces.

## When to use it
A Model Context Protocol (MCP) server that provides AI assistants with access to the Hevy fitness tracking API. This allows you to log workouts, manage routines, browse exercises, and track your fitness progress directly through AI chat interfaces.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/up0otyuqau) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
