---
name: dennisgl-playwright-scraper
type: mcps
description: >
  Leverages Playwright and BeautifulSoup to enable robust web scraping and content extraction, converting complex JavaScript-heavy web pages into high-quality Markdown with browser automation capabilities.
source_repo: dennisgl/mcp-playwright-scraper
source_url: https://github.com/dennisgl/mcp-playwright-scraper
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 11
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 8
pushed_at: "2025-03-09T04:35:18Z"
---
## What it is
MCP server `Playwright Scraper`, catalogued on PulseMCP. Leverages Playwright and BeautifulSoup to enable robust web scraping and content extraction, converting complex JavaScript-heavy web pages into high-quality Markdown with browser automation capabilities.

## When to use it
Leverages Playwright and BeautifulSoup to enable robust web scraping and content extraction, converting complex JavaScript-heavy web pages into high-quality Markdown with browser automation capabilities.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/dennisgl/mcp-playwright-scraper

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/dennisgl-playwright-scraper). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
