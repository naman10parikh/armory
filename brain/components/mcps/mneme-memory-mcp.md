---
name: mneme-memory-mcp
type: mcps
description: >
  A local-first shared memory layer for MCP-aware agents like Claude, Codex, and Hermes, enabling persistent memory across chats and clients via Markdown files and SQLite FTS.
source_repo: ethos-zero/mneme-memory-mcp
source_url: https://github.com/ethos-zero/mneme-memory-mcp
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
A local-first shared memory layer for MCP-aware agents like Claude, Codex, and Hermes, enabling persistent memory across chats and clients via Markdown files and SQLite FTS.

## When to use it
A local-first shared memory layer for MCP-aware agents like Claude, Codex, and Hermes, enabling persistent memory across chats and clients via Markdown files and SQLite FTS.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/xmdiwt2qf6) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
