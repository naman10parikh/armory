---
name: companion-mcp
type: mcps
description: >
  An MCP server for Bitfocus Companion that enables AI assistants to program and control Stream Deck surfaces and other Companion-connected devices. It provides 53 tools for button control, styling, page management, variable handling, and batch operations with verification and rollback.
source_repo: drohi-r/companion-mcp
source_url: https://github.com/drohi-r/companion-mcp
license: Apache 2.0
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 5
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
An MCP server for Bitfocus Companion that enables AI assistants to program and control Stream Deck surfaces and other Companion-connected devices. It provides 53 tools for button control, styling, page management, variable handling, and batch operations with verification and rollback.

## When to use it
An MCP server for Bitfocus Companion that enables AI assistants to program and control Stream Deck surfaces and other Companion-connected devices. It provides 53 tools for button control, styling, page management, variable handling, and batch operations with verification and rollback.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/lm8bvyrjo2) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
