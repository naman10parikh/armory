---
name: dice-roll
type: mcps
description: >
  Provides a simple dice rolling tool that generates random rolls with customizable dice faces and quantities, returning both individual results and their sum.
source_repo: shimapon/mcp-server-diceroll
source_url: https://github.com/shimapon/mcp-server-diceroll
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 1
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 0
pushed_at: "2025-04-05T12:51:24Z"
---
## What it is
MCP server `Dice Roll`, catalogued on PulseMCP. Provides a simple dice rolling tool that generates random rolls with customizable dice faces and quantities, returning both individual results and their sum.

## When to use it
Provides a simple dice rolling tool that generates random rolls with customizable dice faces and quantities, returning both individual results and their sum.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/shimapon/mcp-server-diceroll

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/dice-roll). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
