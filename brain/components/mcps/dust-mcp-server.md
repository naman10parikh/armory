---
name: dust-mcp-server
type: mcps
description: >
  A Model Context Protocol (MCP) server for Dust.tt agents, designed for seamless integration with Claude Desktop via STDIO. Provides robust agent querying, listing, and configuration tools.
source_repo: ma3u/dust-mcp-server
source_url: https://github.com/ma3u/dust-mcp-server
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 7
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 3
pushed_at: "2025-05-28T08:29:55Z"
---
## What it is
A Model Context Protocol (MCP) server for Dust.tt agents, designed for seamless integration with Claude Desktop via STDIO. Provides robust agent querying, listing, and configuration tools.

## When to use it
A Model Context Protocol (MCP) server for Dust.tt agents, designed for seamless integration with Claude Desktop via STDIO. Provides robust agent querying, listing, and configuration tools.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/mw81ni1qmm) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
