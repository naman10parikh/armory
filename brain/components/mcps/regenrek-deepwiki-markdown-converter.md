---
name: regenrek-deepwiki-markdown-converter
type: mcps
description: >
  Converts DeepWiki repositories into well-formatted Markdown, maintaining links between pages while removing headers, footers, navigation elements, and ads for clean documentation extraction.
source_repo: regenrek/deepwiki-mcp
source_url: https://github.com/regenrek/deepwiki-mcp
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 1383
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 81
pushed_at: "2026-03-20T22:38:26Z"
---
## What it is
MCP server `DeepWiki Markdown Converter`, catalogued on PulseMCP. Converts DeepWiki repositories into well-formatted Markdown, maintaining links between pages while removing headers, footers, navigation elements, and ads for clean documentation extraction.

## When to use it
Converts DeepWiki repositories into well-formatted Markdown, maintaining links between pages while removing headers, footers, navigation elements, and ads for clean documentation extraction.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/regenrek/deepwiki-mcp

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/regenrek-deepwiki-markdown-converter). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
