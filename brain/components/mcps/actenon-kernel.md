---
name: actenon-kernel
type: mcps
description: >
  MCP server that verifies proof-bound consequential execution at the edge, acting as the open trust anchor for the Actenon ecosystem. It validates PCCB proofs without issuing grants or making policy decisions.
source_repo: Actenon/actenon-kernel
source_url: https://github.com/Actenon/actenon-kernel
license: Apache 2.0
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 2
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 2
pushed_at: "2026-07-26T23:37:51Z"
---
## What it is
MCP server that verifies proof-bound consequential execution at the edge, acting as the open trust anchor for the Actenon ecosystem. It validates PCCB proofs without issuing grants or making policy decisions.

## When to use it
MCP server that verifies proof-bound consequential execution at the edge, acting as the open trust anchor for the Actenon ecosystem. It validates PCCB proofs without issuing grants or making policy decisions.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/a23wym2sse) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
