---
name: mcquest-mcp
type: mcps
description: >
  A read-only MCP server that gives AI coding agents structured access to a project's source code, architecture, documentation, and Git context through 16 tools for searching, reading, and comparing evidence without modifying files.
source_repo: LionTechSupport/mcquest-mcp
source_url: https://github.com/LionTechSupport/mcquest-mcp
license: unknown
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 0
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-08-25T22:32:24Z"
---
## What it is
A read-only MCP server that gives AI coding agents structured access to a project's source code, architecture, documentation, and Git context through 16 tools for searching, reading, and comparing evidence without modifying files.

## When to use it
A read-only MCP server that gives AI coding agents structured access to a project's source code, architecture, documentation, and Git context through 16 tools for searching, reading, and comparing evidence without modifying files.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/igo4lzy9cx) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
