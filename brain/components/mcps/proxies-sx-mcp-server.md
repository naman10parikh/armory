---
name: proxies-sx-mcp-server
type: mcps
description: >
  Enables AI agents to autonomously buy and operate mobile proxies via MCP, supporting both API key and on-chain USDC payments for tasks like port creation, IP rotation, and traffic management.
source_repo: bolivian-peru/proxies-sx-mcp-server
source_url: https://github.com/bolivian-peru/proxies-sx-mcp-server
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: null
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
Enables AI agents to autonomously buy and operate mobile proxies via MCP, supporting both API key and on-chain USDC payments for tasks like port creation, IP rotation, and traffic management.

## When to use it
Enables AI agents to autonomously buy and operate mobile proxies via MCP, supporting both API key and on-chain USDC payments for tasks like port creation, IP rotation, and traffic management.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/nwsu9yf0qx) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
