---
name: mcp-deepidv
type: mcps
description: >
  deepidv MCP is an MCP server that exposes the deepidv AI-powered verification engine’s operations as structured tools for MCP-compatible clients. It enables AI agents to perform identity verification, face liveness, deepfake detection, AML screening, and more via remote OAuth-based access.
source_repo: Deep-Identity-Inc/agent-skills
source_url: https://github.com/Deep-Identity-Inc/agent-skills
license: Apache 2.0
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 3
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
---
## What it is
deepidv MCP is an MCP server that exposes the deepidv AI-powered verification engine’s operations as structured tools for MCP-compatible clients. It enables AI agents to perform identity verification, face liveness, deepfake detection, AML screening, and more via remote OAuth-based access.

## When to use it
deepidv MCP is an MCP server that exposes the deepidv AI-powered verification engine’s operations as structured tools for MCP-compatible clients. It enables AI agents to perform identity verification, face liveness, deepfake detection, AML screening, and more via remote OAuth-based access.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/prp3wiokca) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
