---
name: john-zhang-xero-accounting
type: mcps
description: >
  Integrates with Xero Accounting Software to access financial data including accounts, transactions, contacts, invoices, and more through authenticated API connections for financial analysis and bookkeeping tasks.
source_repo: john-zhang-dev/xero-mcp
source_url: https://github.com/john-zhang-dev/xero-mcp
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 21
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 14
pushed_at: "2026-09-01T21:05:48Z"
---
## What it is
MCP server `Xero Accounting`, catalogued on PulseMCP. Integrates with Xero Accounting Software to access financial data including accounts, transactions, contacts, invoices, and more through authenticated API connections for financial analysis and bookkeeping tasks.

## When to use it
Integrates with Xero Accounting Software to access financial data including accounts, transactions, contacts, invoices, and more through authenticated API connections for financial analysis and bookkeeping tasks.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/john-zhang-dev/xero-mcp

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/john-zhang-xero-accounting). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
