---
name: mcollina-ripgrep
type: mcps
description: >
  Provides high-performance text search capabilities by wrapping ripgrep, enabling powerful file exploration, pattern matching, and content discovery across local filesystems.
source_repo: mcollina/mcp-ripgrep
source_url: https://github.com/mcollina/mcp-ripgrep
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 74
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 21
pushed_at: "2025-04-29T21:59:02Z"
---
## What it is
MCP server `Ripgrep`, catalogued on PulseMCP. Provides high-performance text search capabilities by wrapping ripgrep, enabling powerful file exploration, pattern matching, and content discovery across local filesystems.

## When to use it
Provides high-performance text search capabilities by wrapping ripgrep, enabling powerful file exploration, pattern matching, and content discovery across local filesystems.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/mcollina/mcp-ripgrep

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/mcollina-ripgrep). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
