---
name: orellazri-coda
type: mcps
description: >
  Provides a bridge between AI and Coda documents, enabling listing, creating, reading, updating, and duplicating pages for collaborative document management and content creation.
source_repo: orellazri/coda-mcp
source_url: https://github.com/orellazri/coda-mcp
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 64
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 34
pushed_at: "2026-03-31T02:54:01Z"
---
## What it is
MCP server `Coda`, catalogued on PulseMCP. Provides a bridge between AI and Coda documents, enabling listing, creating, reading, updating, and duplicating pages for collaborative document management and content creation.

## When to use it
Provides a bridge between AI and Coda documents, enabling listing, creating, reading, updating, and duplicating pages for collaborative document management and content creation.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/orellazri/coda-mcp

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/orellazri-coda). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
