---
name: regressguard
type: mcps
description: >
  RegressGuard is an MCP server that records a known-good baseline and lets AI coding agents detect regressions (broken API contracts, failing tests, schema changes) during their edit loop, allowing them to self-correct before committing.
source_repo: Bharath-code/regressGuard
source_url: https://github.com/Bharath-code/regressGuard
license: MIT License
cli_compat: [claude, cursor, codex, opencode, gemini]
maturity: experimental
stars: 1
eval_score: null
verified_at: 2026-05-27
related: []
tags: [glama, mcp]
forks: 0
pushed_at: "2026-07-18T16:06:36Z"
---
## What it is
RegressGuard is an MCP server that records a known-good baseline and lets AI coding agents detect regressions (broken API contracts, failing tests, schema changes) during their edit loop, allowing them to self-correct before committing.

## When to use it
RegressGuard is an MCP server that records a known-good baseline and lets AI coding agents detect regressions (broken API contracts, failing tests, schema changes) during their edit loop, allowing them to self-correct before committing.

## How to install / invoke
See [Glama](https://glama.ai/mcp/servers/zpqkz52sbo) for the install config.

## Notes
Discovered via the Glama MCP registry (live API). Pending verify -> promote.
