---
name: liorfranko-chain-of-thought
type: mcps
description: >
  Task management system that converts natural language into organized development tasks with dependency tracking, implementation guides, and verification criteria through structured reasoning phases.
source_repo: liorfranko/mcp-chain-of-thought
source_url: https://github.com/liorfranko/mcp-chain-of-thought
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 21
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 7
pushed_at: "2025-05-07T08:48:51Z"
---
## What it is
MCP server `Chain of Thought Task Manager`, catalogued on PulseMCP. Task management system that converts natural language into organized development tasks with dependency tracking, implementation guides, and verification criteria through structured reasoning phases.

## When to use it
Task management system that converts natural language into organized development tasks with dependency tracking, implementation guides, and verification criteria through structured reasoning phases.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/liorfranko/mcp-chain-of-thought

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/liorfranko-chain-of-thought). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
