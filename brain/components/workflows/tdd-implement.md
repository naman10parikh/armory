---
name: tdd-implement
type: workflows
description: >
  Implements Test-Driven Development by analyzing feature requirements,
  creating tests first (red), implementing minimal passing code (green), and
  refactoring while maintaining tests.
source_repo: jerseycheese/Narraitor
source_url: https://github.com/jerseycheese/Narraitor/blob/feature/issue-227-ai-suggestions/.claude/commands/tdd-implement.md
license: MIT
cli_compat: [claude]
maturity: beta
verified_at: 2026-05-26
related: []
tags: [slash-command, code-analysis-testing]
---
## What it is
A community workflow/command by jerseycheese, catalogued in awesome-claude-code. Implements Test-Driven Development by analyzing feature requirements, creating tests first (red), implementing minimal passing code (green), and refactoring while maintaining tests.

## When to use it
Implements Test-Driven Development by analyzing feature requirements, creating tests first (red), implementing minimal passing code (green), and refactoring while maintaining tests.

## Source
From [`jerseycheese/Narraitor`](https://github.com/jerseycheese/Narraitor/blob/feature/issue-227-ai-suggestions/.claude/commands/tdd-implement.md) — see the linked resource for full implementation and usage.
