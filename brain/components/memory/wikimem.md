...
type: memory
description: >
  Use to give an agent a queryable wiki knowledge base when memory should be a navigable knowledge graph, not just a
  flat log: ingest files, folders, and URLs into a linked vault, then search or ask it in natural language.
