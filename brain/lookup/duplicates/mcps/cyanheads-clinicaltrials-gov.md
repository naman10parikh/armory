---
name: cyanheads-clinicaltrials-gov
type: mcps
description: >
  Integrates with ClinicalTrials.gov REST API to search clinical trials by conditions, interventions, locations, and status, plus retrieve detailed study information by NCT ID with automatic data cleaning and local backup storage.
source_repo: cyanheads/clinicaltrialsgov-mcp-server
source_url: https://github.com/cyanheads/clinicaltrialsgov-mcp-server
license: unknown
cli_compat: [claude, codex, cursor, gemini, opencode]
maturity: beta
stars: 91
verified_at: 2026-05-26
related: []
tags: [mcp, pulsemcp]
forks: 24
pushed_at: "2026-08-21T22:39:13Z"
renamed_to: cyanheads/clinicaltrialsgov-mcp-server
---
## What it is
MCP server `ClinicalTrials.gov`, catalogued on PulseMCP. Integrates with ClinicalTrials.gov REST API to search clinical trials by conditions, interventions, locations, and status, plus retrieve detailed study information by NCT ID with automatic data cleaning and local backup storage.

## When to use it
Integrates with ClinicalTrials.gov REST API to search clinical trials by conditions, interventions, locations, and status, plus retrieve detailed study information by NCT ID with automatic data cleaning and local backup storage.

## How to install / invoke
See the source for the `mcpServers` config block (command + args). Source: https://github.com/cyanheads/clinicaltrialsgov-mcp-server

## Notes
Discovered via the PulseMCP registry (https://www.pulsemcp.com/servers/cyanheads-clinicaltrials-gov). License not declared in registry metadata — confirm before production use. Pending verify -> promote.
