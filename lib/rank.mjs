...
export function computeRows(components) {
  ...
  const list = (components || []).map((c, i) => {
    ...
    const component = COMPONENT[c.type] || c.type || "other";
    ...
    return {
      ...
      fits: fitsShelf({ name: c.name, component, description: c.description }),
...
const TERMINAL = {
  ...
  allow: ["crawl4ai", "firecrawl-firecrawl", "claudectx", "vibe-log"],
...
export const SHELF_FIT = {
  ...
  workflow: {
    ...
    deny: ["setup-monorepo", "system-dynamics-modeler"],
...
// PURE: does this row do its component's job? Always true outside the components SHELF_FIT names.
export function fitsShelf({ name = "", component = null, description = "" } = {}) {
...
export function rankRows(scored, { component = null, domain = null, vertical = null, sort = "universal", dir = "desc", limit = 200 } = {}) {
  ...
  // "memory", the one shelf whose name is the same in both forms.
  ...
  const rule = component ? SHELF_FIT[component] : null;
  ...
  return { items: items.slice(0, limit).map(flat), total: items.length, sort, dir, component, domain, vertical, fit, facets: facetsOf(scored) };
}
