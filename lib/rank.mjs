...
const DOMAINS = {
  ...
  auth: ["auth", "oauth", "clerk", "jwt", "login", "sso", "identity", "credential", "rbac"],
...
// "billing" or "stripe" it is billing (CP138 PR F). "Session" alone is usually a terminal, chat or agent
// session; beside "login" or "oauth" it is a signed-in one (CP138 PR H).
const SUPPORTING = { payments: ["subscription"], auth: ["session"] };
...
// Words that begin like a domain word but mean something else do not count (CP138 PR H): "author", "authority"
// and "authentic" are not auth, though "authorization" and "authentication" are; "report" and "repost" are not a
// repo; and Exa counts only as the whole word, never in "example", "exact" or "exam".
const NOT_A_FORM = { auth: /^auth(?:or(?!i[sz])|entic(?!at))/, repo: /^repo(?:rt|st)/, exa: /^exa[a-z0-9]/ };
const atWordStart = (t, k) => {
  for (let i = t.indexOf(k); i !== -1; i = t.indexOf(k, i + 1)) {
    const c = i === 0 ? "" : t[i - 1];
    if ((c >= "a" && c <= "z") || (c >= "0" && c <= "9")) continue;
    if (!NOT_A_FORM[k]?.test(t.slice(i, i + 16))) return true;
  }
  return false;
};
const domainOf = (text) => {
