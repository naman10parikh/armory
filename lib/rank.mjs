...
const WEIGHTS = { tested: 1.4, mentions: 1.2, stars: 1.0 };
...
export function rows() {
  ...
  _rows = (cat.components || []).map((c) => {
    const text = [c.name, c.description, c.tags].filter(Boolean).join(" ");
    return {
      name: c.name,
      component: COMPONENT[c.type] || c.type || "other",
      domain: domainOf(text),
      url: c.source_url || (typeof c.source_repo === "string" ? c.source_repo : null),
      license: c.license,
      desc: (c.description || "").slice(0, 160),
      signals: { stars: stars(c.stars), tested: c.eval_score, mentions: c.mentions ?? null },
    };
  });
  score(_rows);
  return _rows;
...
export function leaderboard({ component = null, domain = null, sort = "universal", dir = "desc", limit = 200 } = {}) {
  let items = rows().filter((r) => (!component || r.component === component) && (!domain || r.domain === domain));
  ...
  return { items: items.slice(0, limit).map(flat), total: items.length, sort, dir, component, domain, facets: facets() };
}

const flat = (r) => ({
  name: r.name, component: r.component, domain: r.domain, url: r.url, license: r.license,
  universal: r.scores.universal, stars: r.signals.stars, tested: r.signals.tested,
  mentions: r.signals.mentions, desc: r.desc,
});

export function facets() {
  const all = rows();
  const count = (f) => { const m = new Map(); for (const r of all) m.set(f(r), (m.get(f(r)) || 0) + 1); return [...m].sort((a, b) => b[1] - a[1]).map(([key, n]) => ({ key, count: n })); };
  return { components: count((r) => r.component), domains: count((r) => r.domain), total: all.length };
}
